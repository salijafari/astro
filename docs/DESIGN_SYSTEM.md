...
**Principle A — Dark celestial, always.** Akhtar is always dark mode. There is no light mode. The visual world is deep space: dark surfaces, translucent cards, animated aurora backgrounds driven by planetary data. Never add a bright background. Never use white as a fill color.

**Principle B — Aurora is ambient, not decorative.** The animated aurora background is a functional element — it reflects which planet is dominant in the user's sky, and its motion speed reflects the lifecycle state of that transit. It is not decoration. Every screen that shows astrological content has a planetary aurora behind it. Screens that are purely utilitarian (settings, profile edit) do not.
...
interface PlanetaryAuroraInput {
  ...
  isStill: boolean;  // true when detail view is open
  opacity?: number;  // default 1.0, use 0.5 for dashboard
}
```
...
- All cards sit at `z-index: 10` and use translucent backgrounds — never opaque
- When `isStill = true`: all motion halts within 600ms using `Easing.bezier(0.25, 0.1, 0.25, 1)`
- When `dominant === null` (no birth profile or no active transits): render Moon palette alone at 0.4× opacity
- Never render a blank dark screen — fallback to Moon palette always

**Extend this component, never fork it.** The existing aurora system in `components/aurora/` is the base. Add the `planetary` variant by extending the existing config, not by creating a parallel component.
...
On top of the planetary aurora, a Moon-phase tint is blended based on current Moon phase. New Moon → darker overlay. Full Moon → brighter. Max opacity: 0.15. Skip this overlay when the dominant planet is Moon itself (to avoid double-up). Phase angle is derived from `moonContext` returned by the API.
...
- House chip: `🏠 7th house · relationships` — `r.pill`, `bg.surface3`, `border.subtle`, `11px`, `text.secondary`
...
**Styling:** `background: bg.surface1`, `border: border.subtle`, `border-radius: r.xl`, `padding: 32px 20px`, `text-align: center`

**Rule:** Always Moon aurora behind the empty state at full opacity — never a blank screen. CTA navigates to the most logical next action (usually profile completion or onboarding).
...
**Sheet styling:** `background: bg.surface2`, `border-radius: r.2xl` on top corners only, handle bar (`36×4px`, `border.strong`, `border-radius: r.pill`, centered, `margin: 12px auto`), `padding: 0 20px 32px`
...
**Transitions:** 150ms fade on active state change. Aurora does not change on tab switch.
...
**Aurora → still mode:** When a detail view opens (`isStill = true`), all aurora layers animate amplitude to `0` over 600ms using `ease.standard`. The planet and palette do not change. When the detail view closes, aurora resumes from current position.

**Tab switch:** Content fades out over 150ms (`ease.accelerate`), new content fades in over 200ms (`ease.decelerate`). No slide. Aurora is unaffected by tab switching.
...
- ❌ Opaque card backgrounds over an aurora background — always translucent
...
2. **Determine the aurora palette** — which planet or context drives the background? See Section 6.4. If the screen is utilitarian (settings, profile edit), no aurora.
...
6. **Test empty and error states** — every screen must have a graceful empty state with Moon aurora fallback.
...
**Rule:** Feature cards on the dashboard never show aurora. The gradient fill is the identity. Aurora only appears behind the dashboard as a whole (at 0.5× opacity from the dominant transit planet), not inside individual feature cards.

### 15.3 Consistency rule

When a user navigates from a dashboard feature card into that feature's screen, the same top color must appear as the accent on chips and CTAs inside the screen. This creates a visual continuity — tap the gold card → enter a screen with gold chips. The transition makes the feature feel cohesive even as the aurora changes behind it.