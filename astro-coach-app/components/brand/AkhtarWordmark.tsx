...
type Props = {
  /** `hero`: onboarding; `home`: sign-in / shared; `authEntry`: welcome auth (compact); `dashboard`: home screen (smaller); `header`: tab bar title */
  size?: "authEntry" | "dashboard" | "header" | "hero" | "home";
};
...
export function AkhtarWordmark({ size = "hero" }: Props) {
  ...
  return (
    <View className={`items-center justify-center ${size === "authEntry" ? "py-1" : "py-2"}`}>
      <Image
