...
/**
 * Converts birth fields in an IANA timezone to UTC Gregorian parts for Swiss Ephemeris.
 */
function toUtcParts(input: NatalChartInput): {
  ...
  const dt = DateTime.fromISO(`${input.birthDate}T${time}:00`, {
    zone: input.birthTimezone,
  });
  if (!dt.isValid) {
    throw new Error(`Invalid birth datetime: ${dt.invalidReason}`);
  }
  const u = dt.toUTC();
  return {
    year: u.year,
...
export function computeNatalChart(input: NatalChartInput): NatalChartResult {
  ...
  if (jd.flag !== constants.OK) {
    throw new Error(jd.error ?? "utc_to_jd failed");
  }
  ...
  for (const { key, id } of BODIES) {
    ...
    if (p.flag !== EPHE_FLAGS) {
      throw new Error(p.error ?? `calc failed for ${key}`);
    }
    const lon = p.data[0];
    longitudes[key] = lon;
    const house =
      input.birthTime != null ? houseForLongitude(lon, cusps) : 0;
    planets.push({
      planet: key,
  ...
  const risingSign =
    input.birthTime != null && asc != null ? signFromLongitude(asc) : null;
...
export function julianNow(): { jdEt: number; jdUt: number } {
  ...
  if (jd.flag !== constants.OK) {
    throw new Error(jd.error ?? "utc_to_jd now failed");
  }
  const [jdEt, jdUt] = jd.data;
  return { jdEt, jdUt };
}
  ...
  for (let i = 0; i < keys.length; i++) {
    for (let j = i + 1; j < keys.length; j++) {
      ...
      for (const t of targets) {
        ...
        if (orb <= t.maxOrb) {
          out.push({ body1: a, body2: b, type: t.type, orb });
        }
      }
    }
  }
  ...
  for (const tBody of Object.keys(transit)) {
    for (const nBody of personal) {
      ...
      for (const { type, angle, maxOrb } of [
        ...
        if (orb <= maxOrb) {
          hits.push({ transitBody: tBody, natalBody: nBody, type, orb });
        }
      }
    }
  }
